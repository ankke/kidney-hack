...
@execution_time
def generate_samples(img_file, path, window=1024, scale=1.0):
    ...
    for i, box in enumerate(boxes):
        ...
        mask_s = mask_s * 255
        cv.imwrite(f'{path}/overlays/{file_id}_{i:03}.png', cv.cvtColor(o, cv.COLOR_RGB2BGR))
        cv.imwrite(f'{path}/masks/{file_id}_{i:03}.png', cv.cvtColor(mask_s, cv.COLOR_GRAY2BGR))
        cv.imwrite(f'{path}/images/{file_id}_{i:03}.png', cv.cvtColor(image_s, cv.COLOR_RGB2BGR))
